...
> 3. **The commercial finding, and it is a supply story.** Measured cost to serve is
...
>    limit — **~5.2 TEAM questions/day, 157/month for the entire account**. One Recovery
>    subscriber is promised 250, so **the free tier cannot host a single paying customer**
>    (~2 if every question woke only one specialist). Lifting it is a Groq tier change, not
